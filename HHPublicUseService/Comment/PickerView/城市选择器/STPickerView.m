...
- (void)setupDefault
{
    // 1.设置数据的默认值
    _title             = nil;
    _font              = MyFont(Font_Regular, 17);
    _titleColor        = HexString(@"#333333");
    _borderButtonColor = HexString(@"#DDDDDD");
    _heightPicker      = 240;
...
- (void)show
{
    ...
    if (self.contentMode == STPickerContentModeBottom) {
        ...
        CGRect frameContent =  self.contentView.frame;
        frameContent.origin.y -= (SCREENH_HEIGHT+self.contentView.st_height)/2;
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            [self.layer setOpacity:1.0];
            self.contentView.frame = frameContent;
        } completion:^(BOOL finished) {
...
- (UIView *)contentView
{
    if (!_contentView) {
        CGFloat contentX = 0;
        CGFloat contentY = SCREENH_HEIGHT;
...
- (UIButton *)buttonLeft
{
    if (!_buttonLeft) {
        ...
        [_buttonLeft setTitleColor:HexString(@"#999999") forState:UIControlStateNormal];
//        [_buttonLeft addBorderColor:self.borderButtonColor];
        [_buttonLeft.titleLabel setFont:MyFont(Font_Regular, 13)];
...
- (UIButton *)buttonRight
{
    if (!_buttonRight) {
        ...
        [_buttonRight setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
//        [_buttonRight addBorderColor:self.borderButtonColor];
        [_buttonRight.titleLabel setFont:MyFont(Font_Regular, 13)];
