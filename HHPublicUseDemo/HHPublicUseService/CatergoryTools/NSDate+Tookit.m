...
+(NSInteger)getWeek_AccordingToYear:(NSInteger)year {
    ...
    NSDateComponents *comps = [self getDateComponents:[self dateFromString:[NSString stringWithFormat:@"%ld-12-31",year] DateFormat:@"yyyy-MM-dd"]];
...
+(NSString*)getWeekRangeDate_Year:(NSInteger)year WeakOfYear:(NSInteger)weekofYear
{
    ...
    if ([self getWeek_AccordingToYear:year] == 53) {
        todayIsWeek += 1;
    }
    ...
    weekDate = [NSString stringWithFormat:@"第%ld周(%@-%@)",weekofYear,[self stringFromDate:firstDayOfWeek DateFormat:@"yyyy年M月d号"],[self stringFromDate:lastDayOfWeek DateFormat:@"yyyy年M月d号"]];
...
+(NSDateComponents *)getCurrentDateComponents {
    return [self getDateComponents:[NSDate date]];
}
+(NSInteger)getCurrentWeek {
    NSInteger week = [[self stringFromDate:[NSDate date] DateFormat:@"w"] intValue];
    return week;
}
+(NSInteger)getCurrentYear{
    NSInteger year = [[self stringFromDate:[NSDate date] DateFormat:@"y"] intValue];
    return year;
}

+(NSInteger)getCurrentQuarter{
    NSInteger quarter = [[self stringFromDate:[NSDate date] DateFormat:@"q"] intValue];
    return quarter;
}

+(NSInteger)getCurrentMonth{
    NSInteger month = [[self stringFromDate:[NSDate date] DateFormat:@"M"] intValue];
    return month;
}

+(NSInteger)getCurrentDay{
    NSInteger day = [[self stringFromDate:[NSDate date] DateFormat:@"d"] intValue];
    return day;
}
